import {TextData, TreeData, RunRecord, RunSource, RunText, ServerEntity, RunEntity, RunRelationship} from './types';
...
function runDFS(node: TreeData, relationships: RunRelationship[], processed_nodes: Set<string>) {
...
export async function recordFeedback(text : TextData, tree: TreeData[]) : Promise<boolean> {
    ...
    try {
        ...
        const response = await fetch("http://cosmos0003.chtc.wisc.edu:9543/record_run", requestOptions);
        if(!response.ok) {
            throw new Error("Server returned response code of " + response.status.toString());
        }
