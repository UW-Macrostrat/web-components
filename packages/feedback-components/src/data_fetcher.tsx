import {Entity, ServerRelationship, ServerResponse, Result} from './types';

function addToMap(paragraph_txt: string, entity_name : string, entity_type: string, terms_map : Map<string, Entity>) {
   ...
   let start_idx = paragraph_txt.toLowerCase().indexOf(entity_name.toLowerCase());
   if(start_idx == -1) {
      return;
   }
...
function mergeChild(src_map : Map<string, Entity>, dst_map : Map<string, Entity>, relationship : ServerRelationship) {
   // Get the child
...
function convertToTree(responseJson : ServerResponse) : Result {
   ...
   for(var curr_entity of responseJson.just_entities) {
      if(curr_entity.entity_type == "strat_name") {
         addToMap(paragraph_txt, curr_entity.entity_name, curr_entity.entity_type, strats_map);
      }
   }
