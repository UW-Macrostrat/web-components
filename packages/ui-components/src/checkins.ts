...
export function createCheckins(props: CheckinProps) {
    ...
    result.forEach((checkin) => {    
        ...
        for (var i = 0; i < checkin.rating; i++) {
            ratingArr.push(h(Icon, { className: "star", icon: "star", style: { color } }));
        }
    
        for(var i = 0; i < 5 - checkin.rating; i++) {
            ratingArr.push(h(Icon, {className: "star", icon: "star-empty", style: {color}}));
        }
        ...
        if (showImage) {
            ...
            image = h("div", { className: 'no-image' }, [
                ...
                h(Icon, {className: 'details-image', icon: "arrow-right", style: {color}})
            ]);
        }
        ...
        let temp = h('div', { 
            ...
            h('div', {className: 'checkin-header'}, [
                ...
                h('div', {className: 'likes-container'}, [
                    h(Icon, {className: 'likes-icon ' + (isDarkMode ? "icon-dark-mode" : ""), icon: "thumbs-up", style: {color}}),
                    h('h3', {className: 'likes'}, checkin.likes),
                ]),
                h('div', {className: 'observations-container'}, [
                    h(Icon, {className: 'observations-icon ' + (isDarkMode ? "icon-dark-mode" : ""), icon: "camera", style: {color}}),
                    h('h3', {className: 'likes'}, checkin.observations.length),
                ]),
                h('div', {className: 'comments-container'}, [
                    h(Icon, {className: 'comments-icon ' + (isDarkMode ? "icon-dark-mode" : ""), icon: "comment", style: {color}}),
