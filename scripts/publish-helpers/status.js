...
export async function getPackagePublicationStatus(data) {
    ...
    if (hasChanges != null && hasChanges) {
        ...
        printChangeInfoForPublishedPackage({ name: data.name, version: info.lastVersion }, true);
    }
    return info;
}
...
async function packageVersionExistsInRegistry(pkg) {
    ...
    if (info == null) {
        console.log(chalk.red("No published version found for " + chalk.bold(pkg.name)));
    }
    ...
    const canPublish = !currentVersionExistsInRegistry && currentVersionChangelogEntry != null;
...
export function notifyUserOfUncommittedChanges(raiseError = true) {
    if (!gitHasChanges())
        return;
    console.log(chalk.bgRed.bold("Error:"), chalk.red.bold("You have uncommitted changes in your git repository."));
    console.log(chalk.red("       You must commit or stash them before publishing."));
    if (raiseError)
        throw new Error("Uncommitted changes in git repository");
}
